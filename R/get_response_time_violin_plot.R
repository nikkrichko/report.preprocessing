#' Return violin plot of response times
#'
#' This function provide response times violin for labels.
...
function(input_dt, scenario_name=NULL, group_by_label=TRUE, interactive=FALSE){
    ...
    if(group_by_label){
      result_plot <-  gp + geom_violin(aes(x=request_name,fill=request_name))
    } else {
      result_plot <-  gp + geom_violin(aes(x=0,y=response_time),fill="#0c4c8a") +
